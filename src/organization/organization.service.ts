...
export class OrganizationService {
  ...
  async getAllChildrenPaginated(
    ...
    const [total, items] = await Promise.all([
      this.childModel.countDocuments(baseFilter),
      this.childModel
        .find(baseFilter)
        .populate('parentId', 'fullName email')
        .sort({ fullName: 1 })
  ...
  async createStaffMember(
    ...
    this.logger.debug(
      `[CREATE STAFF] Organization found. currentStaffCount=${org.staffIds?.length || 0}`,
    );
    ...
    const careProviderType = specialistRoles.includes(createStaffDto.role)
      ? createStaffDto.role
      : undefined;
    ...
    await staff.save();
    this.logger.log(
      `[CREATE STAFF] Staff member created with role=${staff.role}`,
    );
    ...
    this.logger.debug(
      `[CREATE STAFF] Organization updated. newStaffCount=${org.staffIds.length}`,
    );
  ...
  async getMyStaff(leaderId: string, page = 1, limit = 50): Promise<User[]> {
    const org = await this.getOrganizationByLeader(leaderId);
    if (!org) {
      throw new NotFoundException('Organization not found');
    }
    return this.getStaff(org._id.toString(), page, limit);
  }

  async getMyStaffPaginated(
    leaderId: string,
    page = 1,
    limit = 50,
  ): Promise<{
    ...
    return this.getStaffPaginated(org._id.toString(), page, limit);
  }

  // Get leader's organization families
  async getMyFamilies(leaderId: string, page = 1, limit = 50): Promise<User[]> {
    const org = await this.getOrganizationByLeader(leaderId);
    if (!org) {
      throw new NotFoundException('Organization not found');
    }
    return this.getFamilies(org._id.toString(), page, limit);
  }

  async getMyFamiliesPaginated(
    leaderId: string,
    page = 1,
    limit = 50,
  ): Promise<{
    ...
    return this.getFamiliesPaginated(org._id.toString(), page, limit);
  }
  ...
  async getMyChildrenPaginated(
    ...
    return this.getAllChildrenPaginated(
      user.organizationId.toString(),
      page,
      limit,
    );
  ...
  > {
    ...
    for (const c of children as unknown as Array<{
      ...
      const cid = String(c._id || c.id).toString();
    ...
    return children.map((c) => {
      ...
      const cid = String(child._id || child.id).toString();
      ...
      return {
        childId: cid,
        childName: String((child as { fullName?: string }).fullName ?? ''),
  ...
  async inviteUserToOrganization(
    ...
    this.logger.debug(
      `[INVITE] Starting organization invitation flow for type=${invitationType}`,
    );
    ...
    if (!user && invitationType === 'staff') {
      ...
      if (!staffRoles.includes(userData.role)) {
        throw new BadRequestException(
          `Cannot invite this user as staff. User role is '${userData.role}'. Staff members must have one of these roles: ${staffRoles.join(', ')}.`,
        );
      }

      this.logger.debug(
        '[INVITE] Creating new unconfirmed staff account from invitation flow',
      );
    ...
    this.logger.debug(
      `[INVITE] Invitation target prepared. role=${user.role} confirmed=${user.isConfirmed}`,
    );
    ...
    if (existingInvitation) {
      this.logger.debug('[INVITE] Cancelling previous pending invitation');
    ...
    this.logger.debug('[INVITE] Invitation created and confirmation token set');
    ...
    if (user.isConfirmed) {
      ...
      this.logger.debug(
        '[INVITE] Existing confirmed user path selected for invitation',
      );
    } else {
      ...
      this.logger.debug(
        '[INVITE] Confirm-account path selected for unconfirmed invitation user',
      );
    }
    ...
    this.logger.log(
      '[INVITE] Organization invitation email queued successfully',
    );

    return { message: 'Invitation sent successfully' };
  }

  private async findMemberInvitationByToken(
    token: string,
    ...
    const type = invitation.invitationType || invitation.type;
    if (!type || type === 'org_leader_invite') {
      throw new BadRequestException('Invalid invitation type');
    }
    return invitation;
  }

  private async ensureInvitationPendingAndNotExpired(
  ...
  async getInvitationPreview(token: string): Promise<{
    ...
    const invitation = await this.findMemberInvitationByToken(token);
    await this.ensureInvitationPendingAndNotExpired(invitation);
    return {
      organizationName: invitation.organizationName ?? 'Organization',
      invitationType:
        (invitation.invitationType || invitation.type) === 'staff'
          ? 'staff'
          : 'family',
  ...
  async acceptInvitation(
    ...
    this.logger.debug('[ACCEPT] Processing organization invitation');
    ...
    this.logger.debug(
      `[ACCEPT] Invitation found. type=${effectiveType} confirmed=${user.isConfirmed}`,
    );
    ...
    if (effectiveType === 'staff') {
      if (!org.staffIds.some((id) => id.toString() === user._id.toString())) {
        ...
        this.logger.debug('[ACCEPT] Added invited user to organization staff');
      }
    } else {
      if (!org.familyIds.some((id) => id.toString() === user._id.toString())) {
        org.familyIds.push(user._id);
        this.logger.debug(
          '[ACCEPT] Added invited user to organization families',
        );
      }
      ...
      this.logger.debug(
        `[ACCEPT] Found ${existingChildren.length} existing children for invited family`,
      );
      if (existingChildren.length > 0) {
        ...
        this.logger.debug(
          `[ACCEPT] Updated ${updateResult.modifiedCount} children with organization link`,
        );
    ...
    this.logger.log(
      '[ACCEPT] Invitation accepted and organization membership linked',
    );
  ...
  async cancelInvitation(
    ...
    if (user && !user.isConfirmed) {
      this.logger.debug(
        '[CANCEL] Deleting unconfirmed user associated with cancelled invitation',
      );
  ...
  async getOrgLeaderInvitationPreview(token: string): Promise<{
    organizationName: string;
    expiresAt: Date;
  }> {
  ...
  async confirmOrgLeaderInvitationAcceptance(
    token: string,
  ): Promise<{ organization: OrganizationDocument; user: UserDocument }> {
    return this.acceptOrgLeaderInvitation(token);
  }

  async confirmOrgLeaderInvitationRejection(token: string): Promise<void> {
    await this.rejectOrgLeaderInvitation(token);
  }
