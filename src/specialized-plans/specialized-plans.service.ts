...
export class SpecializedPlansService {
  ...
  > {
    ...
    return (plans as unknown as Array<Record<string, unknown>>).map((p) => ({
      planId: (p._id as { toString(): string })?.toString?.() ?? '',

      type: String((p.type as string | number | undefined) ?? ''),

      title: String((p.title as string | number | undefined) ?? ''),
      ...
      lastUpdated: p.updatedAt
        ? new Date(String(p.updatedAt as string | number | Date)).toISOString()
        : undefined,
    }));
  }
