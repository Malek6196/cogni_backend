...
import { SpecializedPlanDocument } from '../specialized-plans/schemas/specialized-plan.schema';
import {
  TaskReminder,
...
export class ProgressContextService {
  ...
  async buildContext(
    ...
    const taskReminders = reminders.map((r: Record<string, unknown>) => {
      const history =
        (r.completionHistory as Array<Record<string, unknown>>) || [];
