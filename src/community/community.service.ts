...
export class CommunityService {
  ...
  async createPost(
    ...
    const user = await this.userModel.findById(uid).select('fullName profilePic').exec();
  ...
  > {
    ...
    const authorIds = [
      ...new Set((posts as PostLean[]).map((p) => p.authorId.toString())),
    ];
  ...
  async getPostsPaginated(
    ...
    const items = (posts as PostLean[]).map((p) => ({
      id: p._id.toString(),
      authorId: p.authorId.toString(),
      authorName: p.authorName,
      authorProfilePic: profilePicByAuthorId.get(p.authorId.toString()) ?? null,
  ...
  async getPostsByAuthorPaginated(
    authorId: string,
    page = 1,
    limit = 50,
  ): Promise<{
    ...
    const skip = (safePage - 1) * safeLimit;
    const [total, posts] = await Promise.all([
      this.postModel.countDocuments({ authorId: oid }).exec(),
      this.postModel
        .find({ authorId: oid })
        .sort({ createdAt: -1 })
        .skip(skip)
    ...
    const users = await this.userModel
      ...
      .exec();
    const profilePic =
      (users as { _id: Types.ObjectId; profilePic?: string }[])[0]
        ?.profilePic ?? null;
  ...
  async getCommentsPaginated(
    ...
    items: { authorName: string; text: string; createdAt: string }[];
    pagination: {
      page: number;
      limit: number;
      total: number;
    ...
    const [total, comments] = await Promise.all([
      this.commentModel.countDocuments({ postId: postObjectId }).exec(),
      this.commentModel
    ...
    const items = (comments as CommentLean[]).map((c) => ({
      authorName: c.authorName,
      text: c.text,
      createdAt: c.createdAt.toISOString(),
    }));
  ...
  async listFriends(
    ...
    const users = await this.userModel
      ...
      .exec();
    return (
      users as { _id: Types.ObjectId; fullName?: string; profilePic?: string }[]
    ).map((u) => ({
      id: u._id.toString(),
  ...
  /** Strict public member DTO for profile previews. */
  async getMemberPublicInfo(userId: string): Promise<{
    fullName: string;
    profilePic?: string;
    role?: string;
    specialty?: string;
    ...
    const user = await this.userModel
      .findById(userId)
      .select('fullName profilePic role specialty')
      .lean()
      .exec();
    if (!user) return null;
    const u = user as {
  ...
  async getFollowStatus(
    ...
    status: FollowRequestStatus | null;
    ...
    const requestId = d.requesterId.equals(me) ? d._id.toString() : null;
    return { status, requestId };
  }

  async listPendingFollowRequests(userId: string): Promise<
    {
      id: string;
      requesterId: string;
    ...
    if (doc.status !== 'pending') {
      if (doc.status === 'accepted') {
        try {
          ...
          this.logger.warn(
            'deleteByFollowRequestId failed on accept (idempotent)',
            e,
          );
        }
        return;
  ...
  async declineFollowRequest(requestId: string, userId: string): Promise<void> {
    const doc = await this.followRequestModel.findById(requestId).exec();
