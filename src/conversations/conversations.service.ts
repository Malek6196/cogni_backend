...
} from '../community/schemas/follow-request.schema';
...
export class ConversationsService {
  ...
  private isFollowExemptRole(roleLower: string): boolean {
    if (!roleLower) return false;
    if (this.isHealthcareRole(roleLower)) return true;
    return roleLower === 'admin';
  }

  private async assertDirectChatPolicy(
    ...
    const otherOid = new Types.ObjectId(otherUserId);
    const users = await this.userModel
      .find({ _id: { $in: [currentOid, otherOid] } })
      .select('_id role blockedUserIds')
      .lean()
      .exec();
    ...
    ).find((u) => u._id.equals(currentOid));
    const other = (
      users as Array<{
        _id: Types.ObjectId;
        role?: string;
    ...
    ).find((u) => u._id.equals(otherOid));

    if (!current || !other) {
      throw new NotFoundException('User not found');
    }

    const currentBlocksOther =
      current.blockedUserIds?.some((id) => id.equals(otherOid)) ?? false;
    const otherBlocksCurrent =
      other.blockedUserIds?.some((id) => id.equals(currentOid)) ?? false;
    if (currentBlocksOther || otherBlocksCurrent) {
      throw new ForbiddenException(
        'Messaging is not allowed for blocked users',
      );
    }

    const currentRole = this.normalizeRole(current.role);
    const otherRole = this.normalizeRole(other.role);
    if (
      this.isFollowExemptRole(currentRole) ||
      this.isFollowExemptRole(otherRole)
    ) {
      return;
    }

    const acceptedFollow = await this.followRequestModel
      .exists({
        status: 'accepted',
        $or: [
          { requesterId: currentOid, targetId: otherOid },
          { requesterId: otherOid, targetId: currentOid },
        ],
      })
      .exec();
    ...
    if (!acceptedFollow) {
      throw new ForbiddenException(
        'Users must have an accepted follow relationship before messaging',
      );
    }
  }
  ...
  private async assertGroupPairPolicy(
    ...
    if (firstUserId.equals(secondUserId)) return;
    await this.assertDirectChatPolicy(
      firstUserId.toString(),
      secondUserId.toString(),
    );
  }

  private async assertGroupCreationPolicy(
    participantIds: Types.ObjectId[],
  ): Promise<void> {
    for (let i = 0; i < participantIds.length; i += 1) {
      for (let j = i + 1; j < participantIds.length; j += 1) {
        await this.assertGroupPairPolicy(participantIds[i], participantIds[j]);
      }
    }
  }

  private async assertCandidateCanJoinGroup(
    candidateId: Types.ObjectId,
    currentParticipantIds: Types.ObjectId[],
  ): Promise<void> {
    for (const existingParticipantId of currentParticipantIds) {
      await this.assertGroupPairPolicy(candidateId, existingParticipantId);
    }
  }

  /** Derive a 32-byte AES key from env, using a dev-only fallback outside production. */
  private getEncryptionKey(): Buffer {
    if (this._encryptionKey) return this._encryptionKey;
    const secret = getMessagesEncryptionSecret(
  ...
  async getOrCreateConversation(
    ...
    await this.assertDirectChatPolicy(userId, otherUserId);
  ...
  async getMessages(
    ...
    const [total, pageMessages] = await Promise.all([
    ...
    const items = [...pageMessages].reverse().map((m) => ({
      id: m._id.toString(),
      senderId: m.senderId.toString(),
      text: this.decryptMessage(m.text),
      createdAt: (m as any).createdAt,
  ...
  async uploadAttachment(
    ...
    const inferImageExtFromBytes = (
      ...
      // HEIC/HEIF: bytes 4..11 often contain "ftypheic"/"ftypheif"/"ftypmif1"/"ftypmsf1"
      if (buf.length >= 12) {
        const box = buf.subarray(4, 12).toString('ascii').toLowerCase();
        if (
    ...
    const ext =
      type === 'voice'
        ...
        : m.includes('png')
          ...
          : m.includes('webp')
            ? 'webp'
            : m.includes('heic') || m.includes('heif')
              ? 'heic'
              : m.includes('jpeg') || m.includes('jpg')
                ? 'jpg'
                : inferImageExtFromBytes(file.buffer);
  ...
  async addMessage(
    ...
    if (!isGroup && conv.otherUserId && conv.user) {
      const otherId = conv.user.equals(uid)
        ? conv.otherUserId.toString()
        : conv.user.toString();
      await this.assertDirectChatPolicy(userId, otherId);
    }
    const encryptedText = this.encryptMessage(text);
    this.logger.log(
      `[ENCRYPTION CHECK] Thread: ${String(threadId)}, Plaintext Length: ${text?.length}, Encrypted Length: ${encryptedText?.length}`,
    );
  ...
  private async emitMessageDeletedToParticipants(
    ...
    const ids = isGroup
      ...
      : ([conv.user?.toString(), conv.otherUserId?.toString()].filter(
          Boolean,
        ) as string[]);
    for (const participantId of ids) {
      ...
      let recipientConversationId = conv._id.toString();
      if (!isGroup) {
        const participantConv = await this.conversationModel
          .findOne({ user: new Types.ObjectId(participantId), threadId })
          .select('_id')
          .lean()
          .exec();
        if (participantConv) {
          recipientConversationId = participantConv._id.toString();
        }
      }
      this.callsGateway.emitMessageDeleted(participantId, {
        conversationId: recipientConversationId,
        messageId,
      });
    }
  ...
  async deleteMessage(
    ...
    if (!msg.threadId.equals(threadId)) {
      throw new ForbiddenException(
        'Message does not belong to this conversation',
      );
    }
    if (!msg.senderId.equals(uid)) {
      throw new ForbiddenException('Only the sender can delete this message');
    }
    await this.messageModel.deleteOne({ _id: msg._id }).exec();
    ...
    const isGroup = conv.participants && conv.participants.length > 0;
    const timeAgo = last
      ? formatTimeAgo(new Date((last as { createdAt?: Date }).createdAt!))
      : '';
    const lastEncrypted = last
      ? ((last as { text?: string }).text ?? this.encryptMessage(''))
      : this.encryptMessage('');
    if (isGroup) {
      await this.conversationModel
        .updateOne(
          { _id: conv._id },
          { lastMessage: lastEncrypted, timeAgo, updatedAt: new Date() },
        )
        .exec();
    } else {
      await this.conversationModel
    ...
    await this.emitMessageDeletedToParticipants(conv, threadId, messageId);
  }
  ...
  async createGroup(
    ...
    await this.assertGroupCreationPolicy(uniqueIds);
  ...
  async addMemberToGroup(
    ...
    if (!Types.ObjectId.isValid(newParticipantId)) {
      throw new BadRequestException('Invalid user id');
    }
    const conv = await this.conversationModel.findById(conversationId).exec();
    if (!conv) throw new NotFoundException('Conversation not found');
    ...
    await this.assertCandidateCanJoinGroup(newId, conv.participants);
