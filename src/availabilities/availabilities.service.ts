...
export class AvailabilitiesService {
  ...
  async listByVolunteerId(volunteerId: string) {
    ...
    return docs.map((d: unknown) => ({
      ...
      volunteerId:
        (
          d as { volunteerId?: { toString?(): string } }
        ).volunteerId?.toString?.() ?? volunteerId,
      dates: (d as { dates?: string[] }).dates ?? [],
      startTime: (d as { startTime?: string }).startTime ?? '14:00',
      endTime: (d as { endTime?: string }).endTime ?? '18:00',
      recurrence: (d as { recurrence?: string }).recurrence ?? 'weekly',
      recurrenceOn: (d as { recurrenceOn?: boolean }).recurrenceOn ?? true,
    }));
  }
