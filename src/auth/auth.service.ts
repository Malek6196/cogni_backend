...
>>>>>>> Stashed changes
  ...
  async login(
    ...
    await this.validateOrganizationLeaderAccess(user);
  ...
  async socialLogin(
    ...
    if (
      verifiedIdentity.provider === SocialProvider.GOOGLE &&
      verifiedIdentity.email &&
      !verifiedIdentity.emailVerified
    ) {
      throw new UnauthorizedException(
        'Social account email is not verified by the provider',
      );
    }
    ...
    if (!user && verifiedIdentity.email) {
      user = await this.userModel.findOne({ email: verifiedIdentity.email });
    }

    if (!user) {
      if (!verifiedIdentity.email) {
        throw new UnauthorizedException(
          'Unable to create account from provider token without email claim',
        );
      }
      ...
      throw new UnauthorizedException(
        'Your account is not confirmed. Please activate your account before signing in.',
      );
    } else {
      user.googleUserId = user.googleUserId ?? verifiedIdentity.providerUserId;
    }
  ...
  private async validateOrganizationLeaderAccess(
    user: UserDocument,
  ): Promise<void> {
  ...
  private async createAuthenticatedResponse(
    ...
    const userResponse: any = {
      ...
      requiresProfileCompletion:
        user.role === 'careProvider' && !user.careProviderType,
    };
  ...
  async completeProfile(
    ...
    if (user.role !== 'careProvider') {
      throw new BadRequestException(
        'Profile completion is only for careProvider users',
      );
    }
    ...
    user.careProviderType = dto.careProviderType;
